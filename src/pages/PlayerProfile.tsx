...
const PlayerProfile = () => {
  ...
  const derivedStats = useMemo(() => {
    ...
    const getBestPlacement = (res: any[]) => {
      ...
      const sorted = [...res].sort((a, b) => (parseInt(a.placement) || 999) - (parseInt(b.placement) || 999));
    ...
    const seasonStats = {
      ...
      wins: seasonResults.reduce((acc: number, r: any) => acc + (r.wins || 0), 0),
      losses: seasonResults.reduce((acc: number, r: any) => acc + (r.losses || 0), 0),
    ...
    const lifetimeStats = {
      ...
      netPoints: results.reduce((acc: number, r: any) => acc + ((r.pf || 0) - (r.pa || 0)), 0)
    };
    ...
    <div className="min-h-screen bg-gray-100 font-sans">
      ...
      <div className="bg-white shadow relative">
            ...
            <div className="absolute top-4 right-4 z-50">
              {isEditing ? (
                <div className="flex gap-2">
                  <button onClick={handleSave} className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded shadow text-sm font-bold">
                    Save
                  </button>
                  <button onClick={handleCancel} className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded shadow text-sm font-bold">
                    Cancel
                  </button>
                </div>
              ) : (
                <button onClick={() => setIsEditing(true)} className="bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded shadow text-sm font-bold">
                  Edit Profile
                </button>
              )}
            </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="mt-4 md:mt-0 md:ml-6 flex-1">
              <div className="flex flex-col md:flex-row md:items-baseline md:justify-between">
                <div>
                  {isEditing ? (
                    ...
                    <h1 className="text-3xl md:text-5xl font-black text-gray-900 uppercase tracking-tight">
                      <span className="text-gray-500 font-bold"> {playerData.firstName} </span> {playerData.lastName}
                    </h1>
                  )}
                    ...
                    {isEditing ? (
                      ...
                      <>
                        <div className="flex items-center">
                          ...
                          />
                          <span className="text-orange-600 font-bold">{playerData.position}</span>
                        </div>
                ...
                <div className="hidden md:block opacity-20">
                  <span className="text-4xl font-black text-gray-400">ICAA</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="h-64 flex items-center justify-center text-gray-500">Loading profile...</div>
        )}
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {activeTab === 'Overview' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-white rounded-lg shadow overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                  <h3 className="text-lg font-bold text-gray-900">2025 - '26 Season Results</h3>
                </div>
                <div className="p-6 grid grid-cols-2 sm:grid-cols-4 gap-6 text-center">
                  <div>
                    <div className="text-3xl font-black text-gray-900">{derivedStats?.seasonStats.tournaments}</div>
                    <div className="text-xs font-bold text-gray-500 uppercase mt-1">Tournaments</div>
                  </div>
                  <div>
                    <div className="text-3xl font-black text-gray-900">{derivedStats?.seasonStats.wins}</div>
                    <div className="text-xs font-bold text-gray-500 uppercase mt-1">Wins</div>
                  </div>
                  <div>
                    <div className="text-3xl font-black text-gray-900">{derivedStats?.seasonStats.losses}</div>
                    <div className="text-xs font-bold text-gray-500 uppercase mt-1">Losses</div>
                  </div>
                  <div>
                    <div className="text-3xl font-black text-gray-900">{derivedStats?.seasonStats.bestPlacement}</div>
                    <div className="text-xs font-bold text-gray-500 uppercase mt-1">Best Placement</div>
                  </div>
                </div>
              </div>
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {derivedStats?.recentTournaments.map((tournament: any, index: number) => (
                        <tr key={index}>
                          <td className="px-6 py-4 font-medium text-gray-900">{tournament.name}</td>
                          <td className="px-6 py-4 text-gray-500">{tournament.city}</td>
                          <td className="px-6 py-4 text-gray-500">{tournament.date}</td>
                          <td className="px-6 py-4 text-gray-500">{tournament.team}</td>
                          <td className="px-6 py-4 font-bold text-gray-900">{tournament.placement}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <div className="space-y-6">
              <div className="bg-white rounded-lg shadow overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-100 bg-gray-50">
                  <h3 className="text-lg font-bold text-gray-900">Lifetime Results</h3>
                </div>
                <div className="p-6 space-y-6 text-center">
                  <div>
                    <div className="text-4xl font-black text-gray-900">{derivedStats?.lifetimeStats.wins}</div>
                    <div className="text-xs font-bold text-gray-500 uppercase mt-1">Wins</div>
                  </div>
                  <div>
                    <div className="text-4xl font-black text-gray-900">{derivedStats?.lifetimeStats.losses}</div>
                    <div className="text-xs font-bold text-gray-500 uppercase mt-1">Losses</div>
                  </div>
                  <div>
                    <div className="text-4xl font-black text-gray-900">{derivedStats?.lifetimeStats.netPoints}</div>
                    <div className="text-xs font-bold text-gray-500 uppercase mt-1">Net Points</div>
                  </div>
                  <div>
                    <div className="text-4xl font-black text-gray-900">{derivedStats?.lifetimeStats.bestPlacement}</div>
                    <div className="text-xs font-bold text-gray-500 uppercase mt-1">Best Placement</div>
                  </div>
        ...
        {activeTab === 'Stats' && (
          <div className="bg-white rounded-lg shadow p-12 text-center">
            <h3 className="text-xl font-bold text-gray-900">Stats to be implemented in the future</h3>
          </div>
        )}
        ...
        {activeTab === 'Tournament Results' && (
          <div className="bg-white rounded-lg shadow overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
              <h3 className="text-lg font-bold text-gray-900">Tournament Results</h3>
              <select
                value={selectedYear}
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-left text-sm">
                <thead className="bg-gray-50 text-gray-500 uppercase font-bold text-xs">
                  <tr>
                    <th className="px-6 py-3 whitespace-nowrap">Tournament</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {playerData.tournamentResults
                    .filter((t: any) => selectedYear === 'All' || t.year === selectedYear)
                    .sort((a: any, b: any) => new Date(b.date).getTime() - new Date(a.date).getTime())
                    .map((result: any, index: number) => (
                    <tr key={index} className="hover:bg-gray-50">
                      <td className="px-6 py-4 font-medium text-gray-900">{result.tournament}</td>
                      <td className="px-6 py-4 text-gray-500">{result.city}</td>
                      <td className="px-6 py-4 text-gray-500">{result.date}</td>
                      <td className="px-6 py-4 text-gray-500">{result.team}</td>
                      <td className="px-6 py-4 text-gray-500">{result.wins}</td>
                      <td className="px-6 py-4 text-gray-500">{result.losses}</td>
                      <td className="px-6 py-4 text-gray-500">{result.pf}</td>
                      <td className="px-6 py-4 text-gray-500">{result.pa}</td>
                      <td className="px-6 py-4 text-gray-500">{(result.pf || 0) - (result.pa || 0)}</td>
                      <td className="px-6 py-4 font-bold text-gray-900">{result.placement}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
